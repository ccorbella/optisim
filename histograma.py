...
def supermercat(T,nl,N):
    ...
    while ((t < T)  or (caixes.sum() > 0)): #Comencem el loop
        ...
        if ((t > 10000) and (t < 43250)):
            espera_clients_v += caixes.sum()*(t-to)

    return np.mean(espera_clients[10000:]), np.mean(temps_espera), espera_clients_v/33250, np.mean(ll_t_cobrar) #mitjana

tespera = np.empty(10000)
tespera2= np.empty(1000)
nespera = np.empty(10000)
nespera2= np.empty(1000)
...
for i in xrange(1000):
    ...
    v2 = np.empty(10)
    for j in xrange(10):
        dades = supermercat(60*60*12,17,20)
        v1[j] = dades[1]
        v2[j] = dades[2]
        tespera[k] = dades[1]
        nespera[k] = dades[2]
        k += 1
    tespera2[i] = np.mean(v1)
    nespera2[i] = np.mean(v2)
    
    print(i)

import scipy.io as sio
sio.savemat('tespera.mat',{'vect':tespera}) 
sio.savemat('nespera.mat',{'vect':nespera}) 
sio.savemat('tespera2.mat',{'vect':tespera2}) 
sio.savemat('nespera2.mat',{'vect':nespera2}) 
