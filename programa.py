    ...
    while (t < T) or (caixes.sum() > 0):
        ...
        if  ((t == propera_entrada) or (caixes.sum() == 0)) and (t < T):
            ...
            if caixes[idx_caixa] > 0:
                properes_sortides[idx_caixa]     = data[idx_caixa][0][0] + temps_cobrar(data[idx_caixa][0][1])
            
            temps_espera                     = np.append(temps_espera,t-element[0])
        ...
        espera_clients[to:t] = caixes.sum()
